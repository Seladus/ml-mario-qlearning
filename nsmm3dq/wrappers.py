...
class MarioRamWrapper(gym.ObservationWrapper):
    ...
    def _enemy_position(self, i):
        if i > 4:
            return -1
        if not self.env.ram[MarioRamWrapper.ENEMY_DRAWN + i]:
            return 0
        return (
            self.env.ram[MarioRamWrapper.ENEMY_POSITION_LEVEL + i] * 0x100
            + self.env.ram[MarioRamWrapper.ENEMY_POSITION_SCREEN + i]
        )

    def _x_position(self):
        return self.env.ram[0x6D] * 0x100 + self.env.ram[0x86]
